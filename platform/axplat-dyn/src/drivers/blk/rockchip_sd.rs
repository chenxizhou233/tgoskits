...
// Try IDMAC for every read/write request. If a transfer fails, the DWMMC core
// recovers the data path before this driver falls back to PIO.
const USE_IDMAC_READ: bool = true;
const USE_IDMAC_WRITE: bool = true;
const USE_MULTI_BLOCK_PIO_READ: bool = true;
...
struct SdBlockQueue {
    ...
    dma: DeviceDma,
    capacity_blocks: u64,
    idmac_success_logged: bool,
    idmac_failure_logged: bool,
}
    ...
    fn create_queue(&mut self) -> Option<alloc::boxed::Box<dyn rd_block::IQueue>> {
        self.dev.take().map(|dev| {
            alloc::boxed::Box::new(SdBlockQueue {
                ...
                idmac_success_logged: false,
                idmac_failure_logged: false,
            }) as _
        })
    }
            ...
            rd_block::RequestKind::Read(mut buffer) => {
                ...
                if USE_IDMAC_READ {
                    match self.try_read_dma(start_block, &mut buffer) {
                        Ok(()) => {
                            if !self.idmac_success_logged {
                                info!(
                                    "rockchip-dwmmc: IDMAC per-request read active, first request \
                                     block={} blocks={}",
                                    start_block, block_count
                                );
                                self.idmac_success_logged = true;
                            }
                            return Ok(rd_block::RequestId::new(0));
                        }
                        Err(err) => {
                            if !self.idmac_failure_logged {
                                warn!(
                                    "rockchip-dwmmc: IDMAC read failed at block={} blocks={}: \
                                     {:?}; falling back to PIO",
                                    start_block, block_count, err
                                );
                                self.idmac_failure_logged = true;
                            }
                        }
                    }
                }
                ...
                if USE_IDMAC_WRITE {
                    match self.try_write_dma(start_block, items) {
                        Ok(()) => {
                            if !self.idmac_success_logged {
                                info!(
                                    "rockchip-dwmmc: IDMAC per-request write active, first \
                        ...
                        Err(err) => {
                            if !self.idmac_failure_logged {
                                warn!(
                                    "rockchip-dwmmc: IDMAC write failed at block={} blocks={}: \
                                     {:?}; falling back to PIO",
                                    start_block, block_count, err
                                );
                                self.idmac_failure_logged = true;
...
impl SdBlockQueue {
    ...
    fn dma_read_into(
        ...
        self.raw
            .host_mut()
            .dma_read_blocks_into(start_block, ptr, size, &self.dma)
    ...
    fn dma_write_from(
        ...
        self.raw
            .host_mut()
            .dma_write_blocks_from(start_block, ptr, size, &self.dma)
            .map_err(map_dev_err_to_blk_err)
    }
    ...
    fn read_blocks_pio(
        ...
        let remaining = self.capacity_blocks.saturating_sub(start_block as u64);
        if remaining == 0 {
            return Err(rd_block::BlkError::InvalidBlockIndex(start_block as usize));
        }
        if block_count > remaining as usize {
            return Err(rd_block::BlkError::InvalidBlockIndex(
                start_block as usize + remaining as usize,
            ));
        }
        if block_count == 1 || !USE_MULTI_BLOCK_PIO_READ {
            for (index, chunk) in out.chunks_exact_mut(BLOCK_SIZE).enumerate() {
                let block = start_block + index as u32;
                let mut block_buf = [0; BLOCK_SIZE];
                self.raw.read_block(block, &mut block_buf).map_err(|err| {
                    warn!(
                        "rockchip-dwmmc: PIO single-block read failed: block={}, err={:?}",
                        block, err
                    );
                    map_dev_err_to_blk_err(err)
                })?;
                chunk.copy_from_slice(&block_buf);
            }
            return Ok(());
        }

        self.raw
            .read_blocks(start_block, block_count as u32, |addr, block| {
                let off = (addr - start_block) as usize * BLOCK_SIZE;
                out[off..off + BLOCK_SIZE].copy_from_slice(block);
            })
            .map_err(|err| {
                warn!(
                    "rockchip-dwmmc: PIO multi-block read failed: block={} blocks={}, err={:?}",
                    start_block, block_count, err
                );
                map_dev_err_to_blk_err(err)
            })?;
