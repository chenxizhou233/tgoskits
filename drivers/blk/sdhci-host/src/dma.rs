...
use sdmmc_protocol::{
    cmd::{DataDirection, cmd17, cmd18, cmd24, cmd25},
...
impl Sdhci {
    ...
    fn dma_read_blocks_mapped(
        ...
        let result = self
            .issue_command(&cmd)
            .and_then(|_| self.wait_data_complete_with_adma(self.active_data_cmd, Phase::DataRead));
        self.use_dma = false;
        result?;
        Ok(())
    }
    ...
    fn dma_write_blocks_mapped(
        ...
        let result = self.issue_command(&cmd).and_then(|_| {
            self.wait_data_complete_with_adma(self.active_data_cmd, Phase::DataWrite)
        });
        self.use_dma = false;
        result?;
        Ok(())
