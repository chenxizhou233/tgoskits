...
use log::warn;
use sdmmc_protocol::{
    cmd::{CMD12, DataDirection, cmd17, cmd18, cmd24, cmd25},
...
impl DwMmc {
    ...
    fn dma_write_blocks_mapped(
        ...
        self.clear_all_int_status();
        self.program_data_phase(BLOCK_SIZE as u32, block_count);
        self.reset_dma_for_phase(Phase::DataWrite)?;
        ...
        if let Err(err) = self.issue_command(&cmd) {
            self.disable_idmac();
            self.recover_after_idmac_error(Phase::DataWrite);
            self.clear_all_int_status();
            return Err(err);
        }

        let result = self.wait_dma_complete(cmd.cmd, Phase::DataWrite);
        if result.is_ok() && block_count > 1 {
            let _ = self.issue_command(&CMD12);
        }
        self.disable_idmac();
